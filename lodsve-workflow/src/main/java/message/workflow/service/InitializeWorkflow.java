...
public class InitializeWorkflow implements ApplicationListener<ContextRefreshedEvent> {
    ...
    private List<FlowNode> initFlowNode(Workflow workflow, List<Element> children) {
        ...
        for (Element child : children) {
            ...
            String code = XmlUtils.getElementAttr(child, Constants.ATTR_CODE);
            ...
            String roles = XmlUtils.getElementBody(XmlUtils.getChild(child, Constants.TAG_ROLES));
    ...
    private void checkNode(List<FlowNode> nodes) {
        for (FlowNode node : nodes) {
            ...
            if (_nodes.size() > 1) {
                throw new RuntimeException("节点【" + node.getName() + "】的code【" + code + "】存在重复！");
            }
            ...
            FlowNode _node = ListUtils.findOne(nodes, new ListUtils.Decide<FlowNode>() {
                @Override
                public boolean judge(FlowNode target) {
                    return to.equals(target.getCode()) || "end".equals(to);
                }
            });

            if (_node == null) {
                throw new RuntimeException("节点【" + node.getName() + "】的下一节点【" + code + "】不存在，请检查！");
            }
        }
    }
    ...
    private Workflow initWorkflow(Resource resource, Document document, Element root) {
        ...
        String code = XmlUtils.getElementAttr(root, Constants.ATTR_CODE);
        ...
        Workflow lastestWorkflow = workflowService.loadLatestWorkflow(code);
        ...
        // 检查处理类是否正确
        Class<?> handlerCLass = checkClass(handler, String.format("解析流程处理类出现问题！流程名：%s, 处理类：%s", name, handler));
        // 检查domain
