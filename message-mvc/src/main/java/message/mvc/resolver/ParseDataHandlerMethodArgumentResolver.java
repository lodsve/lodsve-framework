...
public class ParseDataHandlerMethodArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    private StringParse generateParseClassBody(Class<?> dest, String parseObjKey) {
        try {
            ...
            //创建一个类
            CtClass ctClass = cp.makeClass("message.context.StringParse$" + System.currentTimeMillis());
            //创建一个接口(StringUtils.StringParse)
            CtClass ctInterface = cp.makeInterface("message.context.StringParse");
            //上面创建的类实现上面的接口
            ...
            if (StringUtils.isEmpty(destClass)) {
                ...
                body.append("return message.context.NumberUtils.isNumber($1) ? ").append(destClass).append(".valueOf($1) : null;\n");
            }
