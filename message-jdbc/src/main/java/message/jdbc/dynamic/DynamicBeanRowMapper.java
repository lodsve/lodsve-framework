...
public class DynamicBeanRowMapper extends ColumnMapRowMapper {
    ...
    private void createDynamicMapper(String string) {
        ...
        try {
            ...
            CtClass ctInterface = cp.makeInterface("message.jdbc.dynamic.DynamicRowMapper");
            //上面创建的类实现上面的接口
            ctClass.addInterface(ctInterface);

            //创建一个字段(SqlHelper)
            CtField sqlHelperField = CtField.make("private message.jdbc.utils.helper.SqlHelper sqlHelper;", ctClass);
            //加入这个字段
            ctClass.addField(sqlHelperField);

            //创建一个setter方法
            CtMethod setterMethod = CtMethod.make("" +
                    "public void setSqlHelper(message.jdbc.utils.helper.SqlHelper sh){" +
                    "	this.sqlHelper = sh;" +
                    "}", ctClass);
            //加入这个方法
            ...
            CtMethod getterMethod = CtMethod.make("" +
                    "public message.jdbc.utils.helper.SqlHelper getSqlHelper(){" +
