...
public class LodsveJackson2JavaTypeMapper extends DefaultJackson2JavaTypeMapper {
    ...
    public JavaType toJavaType(MessageProperties properties) {
        ...
        if (StringUtils.isEmpty(keyTypeId)) {
            return JSON_OBJECT_MAPPER.getTypeFactory().constructParametricType(forName(classTypeId), forName(classTypeId), forName(contentTypeId));
        } else {
            return JSON_OBJECT_MAPPER.getTypeFactory().constructParametricType(forName(classTypeId), null, forName(keyTypeId), forName(contentTypeId));
        }
    }
